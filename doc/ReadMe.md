...
## 2.1 使用vagrant快速创建linux虚拟机

[06、环境-使用vagrant快速创建linux虚拟机](https://www.bilibili.com/video/BV1np4y1C7Yf/?spm_id_from=333.788.videopod.episodes&vd_source=0a0dd058ef849bffba564af91a70780d&p=6)

1 下载&安装VirtualBox

1） https://www.virtualbox.org/，要开启CPU虚拟化

2）旧版本下载地址，win11可能与最新版本的不兼容，安装了VirtualBox-6.0.12

https://www.virtualbox.org/wiki/Download_Old_Builds

> 注意: 说一下版本,因为不兼容,容易搭建不出环境
>
> win11
>
> VirtualBox 6.1.48
...
```shell
# 1 可初始化一个centos7系统,执行完上面的命令后，会在用户的家目录下生成Vagrantfile文件。
$ vagrant init centos/7
# 2 启动虚拟机vagrant，系统vagrant用户的密码是vagrant。下载镜像过程比较漫长，也可以采用先用下载工具下载到本地后，然后使用“ vagrant box add ”添加，再“vagrant up”即可
#   出现以下语句，表示启动成功：==> default: Rsyncing folder: /cygdrive/c/Users/Administrator/ => /vagrant
$ vagrant up
# 2附 将下载的镜像添加到virtualBox中
$ vagrant box add centos/7 E:\迅雷下载\CentOS-7-x86_64-Vagrant-1905_01.VirtualBox.box
# 3 开启SSH，使用vagrant用户连接虚拟机 ，并登陆到centos7
$ vagrant ssh
```
...
```
config.vm.network "private_network", ip: "192.168.56.10"e
```
...
注意
目前官方不支持centos7了，在下载软件之前，在 CentOS 7 虚拟机中执行以下命令，切换为归档源
...
执行命令失效时候，可以试试配置dns：sudo bash -c 'echo "nameserver 8.8.8.8" > /etc/resolv.conf'
...
注意：修改源，才能下载docker-ce docker-ce-cli 
...
7）配置docker镜像加速(https://zhuanlan.zhihu.com/p/24461370776)

阿里云，容器镜像服务 针对Docker客户端版本大于1.10.0的用户 您可以通过修改daemon配置文件/etc/docker/daemon.json来使用加速器

加速地址：https://docker.xuanyuan.me(当前可用，后续如果没有用了，在找)

- ```text
  docker pull docker.xuanyuan.me/library/mysql:5.7  // 待测试
  ```
...
8） 注意，如果安装有出现403，可能是网络配置的问题，需要将网络设置为下面的。每次重启docker，可能都会重置，所以下载的时候，查看一下这个文件
...
```shell 
[root@hadoop-104 module]# sudo docker pull mysql:5.7
```

2 使用命令查看docker里面所存在软件，即查看镜像
...
启动mysql, mysql端口映射到主机(这个主机指的是centos 7)，其重要文件，挂载到主机上面。
注意：需要注意的是，如果使用下面的命令进行一个实例创建并且启动的话，需要保证/mydata/mysql/log、/mydata/mysql/data、/mydata/mysql/conf三个文件夹存在，否则创建完实例之后，启动不了。
...
>
>-e MYSQL_ROOT_PASSWORD=root：初始化 root 用户的密
...
[root@hadoop-104 conf]# cat my.cnf   (touch m.cfn创建目录)
...
[root@hadoop-104 conf]# docker restart mysql  (重启)
```
...
3）webstorm设置maven的配置

![image-20200422211607028](newImgs/idea_maven_setting.png)
